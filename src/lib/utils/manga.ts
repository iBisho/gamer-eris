...
export function mangaAlert(subscription: GamerManga, title: string, image: string, data: MangaAlertData[]) {
  const payload = data.map(d => `**[${d.text}](${d.url})**`).join('\n')

  const embed = new MessageEmbed().setTitle(title).setDescription(payload).setImage(image).setTimestamp()
