...
export default new Command([`disable`], async (message, args, context) => {
  ...
  if (!commandName) return helpCommand?.execute(message, [`disable`], { ...context, commandName: 'help' })
  ...
  if (commandName.toLowerCase() === 'allcommands') {
    ...
    if (!perms) return message.channel.createMessage(language(`settings/setpermission:ALREADY_DISABLED_ALL_COMMANDS`))
    ...
    if (!targets.length) {
      ...
      return message.channel.createMessage(language(`settings/setpermission:DISABLED_ALL_COMMANDS`))
    }
    ...
    if (roleIDs.length) {
      perms.exceptionRoleIDs = processPermChange(false, perms.enabled, roleIDs, perms.exceptionRoleIDs)
    }
  ...
  if (commandName.toLowerCase() === 'tagusage') {
    ...
    if (!perms) return message.channel.createMessage(language(`settings/setpermission:ALREADY_DISABLED_TAG_USAGE`))
    ...
    if (!targets.length) {
      ...
      return message.channel.createMessage(language(`settings/setpermission:DISABLED_TAG_USAGE`))
    }
  ...
  if (!command) return helpCommand?.execute(message, [`disable`], { ...context, commandName: 'help' })
